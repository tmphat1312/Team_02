...
import { db } from "./";
import { userTable, sessionTable, accountTable, verificationTable } from './schema';
...
async function seedDatabase() {
	...
	await Promise.resolve().then(async () => {
		await Promise.all([
			...
			db.insert(sessionTable).values(session as typeof sessionTable.$inferInsert[]),
			db.insert(accountTable).values(account as typeof accountTable.$inferInsert[]),
		]);
